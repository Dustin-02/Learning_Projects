...
start=[[4,8,7],[1,0,6],[2,3,5]]  #fixed starting position:must be same SIZExSIZE
SCRAMBLE=False    #if false then use the start[] data as starting position, 
...
class Solver4(Solver):
    """
    Concrete solver 4: Uses Priority Queue with Manhattan distance heuristic
    """
    def __init__(self, goal, width, height):
        self.pq = Q.PriorityQueue()
        self.goal = goal
    ...
    def add(self, board):
        value = self.manhattan_distance(board)
        self.pq.put((value, board))
    ...
    def manhattan_distance(self, board):
        ...
        for x in range(self.w):
            for y in range(self.h):
                if board[x][y] != 0:  # Skip blank tile
                    goal_x, goal_y = goal_positions[board[x][y]]
...
class Board:
    ...
    def __init__(self, bwidth, bheight):
        ...
        #self.Solver=Solver4(self.goal,bwidth,bheight)
        #####################################
...
class Animation:
            ...
            if not(pause):
                 if not(end):
                      ...
                      if move is not None:
                          ...
                          self.slideAnimation(board,move,10)
                      else:
                          end=True
                          # Print final metrics when puzzle is solved
                          end_time = time.time()
                          print(f"\nFinal Metrics:")
                          print(f"Time taken: {end_time - self.start_time:.3f} seconds")
                          print(f"Moves made: {self.moves_count}")
    ...
    def drawBoard(self, board):
        ...
        txtsurf=font.render("Dr Loke: Introduction to AI", True, (255,255,255))
...
# def compare_solvers(initial_board, size=3):
#     # Create instances
#     board = Board(size, size)
#     solver3 = Solver3(board.goal, size, size)
#     solver4 = Solver4(board.goal, size, size)
    
#     results = {}
#     for name, solver in [("Solver3", solver3), ("Solver4", solver4)]:
#         # Reset board
#         board.board = deepcopy(initial_board)
...
#         # Run solver
#         for board_state, move in board.solve():
#             if move is None:
#                 break
#             moves += 1
#             states_explored += 1
...
# results = compare_solvers(start)

# # Print results
# for solver, metrics in results.items():
#     print(f"\n{solver} Results:")
#     print(f"Time taken: {metrics['time']:.3f} seconds")
#     print(f"Moves required: {metrics['moves']}")
#     print(f"States explored: {metrics['states']}")

def log_metrics(board):
    """
    Log and print metrics for the current solver without consuming the generator
    """
    # Get solver name
    solver_name = board.Solver.__class__.__name__
    ...
    print(f"Goal state:")
    board.printBoard(board.goal)
    print(f"Heuristic type: {board.Solver.heuristic.__name__ if hasattr(board.Solver, 'heuristic') else 'None'}")
    print(f"{'='*50}\n")

def main():
    game = Animation(SIZE, SIZE)
    # Add this line after game initialization
    log_metrics(game.Board)
    
    # Modify the Animation class's run method to track metrics
    start_time = time.time()
    moves_count = 0
    states_explored = 0
    ...
    print(f"Moves made: {moves_count}")
